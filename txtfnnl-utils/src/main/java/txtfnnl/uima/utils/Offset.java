...
public class Offset implements Comparable<Offset> {
   ...
   * while with equal starts, the one with the larger end value is considered smaller for ordering.
   * If the starts and ends are equal, the Offset with the first value that is larger than the
   * other's is ordered first.
   * 
  ...
  public boolean contains(Offset other) {
    ...
    // other is not within this Offset:
    if (!Offset.contains(start(), end(), other.start(), other.end())) return false;
    // other must have length 1 or 2, within this Offset:
    else if (values.length == 2 && oLen < 3) return true;
    // other must be an Offset at the same position:
