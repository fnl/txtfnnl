/**
 * This package provides the necessary classes to extract text from files that is more accessible
 * for language processing than the text structure created by the default Tika SAX handlers (for a
 * few, chosen file types) and wraps the Tika extractor into the UIMA framework.
 */
...
/**
 * A wrapper for the {@link org.apache.tika.Tika} facade that re-packs some of the functionality
 * but changes the behavior of the underlying parsers and content extraction. In addition,
 * documents by default may have up to 1M characters (Tika's original default is 100k), which is in
 * the range of small books.
 * <p>
 * The following input formats have significantly changed behavior:
 * <ul>
...
public class TikaWrapper {
   ...
   * memory problems with huge input documents. The default setting is 1M characters (ten times
   * more than Tika's default and enough to parse a small-sized book).
   */
  ...
  /**
   * Creates a Tika wrapper using the given detector instance and the default parser configuration.
   * 
   * @param detector type detector
   */
   ...
   * <p>
   * If the document stream supports the {@link InputStream#markSupported() mark feature}, then the
   * stream is marked and reset to the original position before this method returns. Only a limited
   * number of bytes are read from the stream.
   * <p>
  ...
  MediaType detect(InputStream stream, Metadata metadata) throws IOException {
    if (stream == null || stream.markSupported()) return detector.detect(stream, metadata);
    else return detector.detect(new BufferedInputStream(stream), metadata);
  }

  /**
   * Append a language profiler to a content handler, returning an updated (tee'd) content handler.
   * 
   * @return a new content handler
   */
  public ContentHandler teeProfiler(ContentHandler handler, ProfilingHandler profiler) {
    return new TeeContentHandler(handler, profiler);
  }

  /**
   * Parse a content stream with the given handler and in the given context, using the provided
   * metadata.
   * 
   ...
   * @see #parse(InputStream, ContentHandler, Metadata, ParseContext)
   */
  public void parse(InputStream stream, ContentHandler handler, ParseContext context)
      throws IOException, TikaException {
   ...
   */
  public void parse(InputStream stream, ContentHandler handler) throws IOException, TikaException {
   ...
   * @see #parse(InputStream, ContentHandler, Metadata)
   */
  public ContentHandler parse(InputStream stream, Metadata metadata) throws IOException,
      TikaException {
    final CleanWriteOutContentHandler handler = new CleanWriteOutContentHandler(maxStringLength);
   ...
   */
  public ContentHandler parse(InputStream stream) throws IOException, TikaException {
  ...
  public ContentHandler parse(URL url) throws IOException, TikaException {
    ...
    final InputStream stream = TikaInputStream.get(url, metadata);
  ...
  public static void main(String[] argv) throws IOException, TikaException {
    final TikaWrapper wrapper = new TikaWrapper();
