...
public abstract class AbstractTikaAnnotator extends JCasAnnotator_ImplBase {
    ...
    /**
     * For XML, optionally use a specific, known handler. The available handlers (String values)
     * are:
     * <ul>
     ...
     * <li>{@link txtfnnl.tika.sax.CleanBodyContentHandler} only removes "ignorable whitespaces"
     * and uses the default Tika XML parser</li>
     * </ul>
     ...
     * plain-text SOFA with a {@link View.CONTENT_TEXT}. It preserves all metadata annotations on
     * the raw content that Tika was able to detect.
     */
    @Override
    public void process(JCas aJCas) throws AnalysisEngineProcessException {
        ...
        if (stream == null) {
            ...
            logger.log(Level.INFO, "parsing {0} from {1}", new Object[] { aJCas.getSofaMimeType(),
        ...
        try {
            ...
            if ("text/html".equals(mediaType) || mediaType.startsWith("application/xhtml")) {
                ...
                if (XMLContentHandler.class.getName().equals(xmlHandlerClass)) {
                    ...
                    logger.log(Level.WARNING, "unknown XML handler {0} - using default",
                        xmlHandlerClass);
                    handler = new XMLContentHandler(handler);
                    parser = new UnembeddedXMLParser();
                }
