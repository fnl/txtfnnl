 ...
 * <p/>
 * The plaintext will contain no "ignorable whitespaces" (see {@link
 * txtfnnl.tika.sax.ToCleanTextContentHandler ToCleanTextContentHandler}) and may have all Greek
 * characters replaced with the character name spelled out using Latin (ASCII) letters (see {@link
 * txtfnnl.tika.sax.GreekLetterContentHandler GreekLetterContentHandler} and {@link
 * AbstractTikaAnnotator#PARAM_NORMALIZE_GREEK_CHARACTERS}).
 * <p/>
 ...
 * of the Tika extraction setup (see {@link AbstractTikaAnnotator#PARAM_ENCODING}). If the input
 * stream MIME type is <code>text/html</code> or starts with <code>application/xhtml</code>, the
 ...
 * For XML, in addition, any other known content handler can be set using the {@link
 * #PARAM_XML_HANDLER} setting (see parameter documentation). For all other MIME types, Tika's
 * {@link AutoDetectParser} will be used. If Tika detects {@link org.apache.tika.metadata.Metadata},
 * it is added as {@link txtfnnl.uima.tcas.DocumentAnnotation} to the output view.
 *
 * @author Florian Leitner
 */
public abstract
class AbstractTikaAnnotator extends JCasAnnotator_ImplBase {
   ...
   * <ul> <li>{@link txtfnnl.tika.sax.XMLContentHandler} introduces line-breaks at element
   * boundaries, the <b>default</b> handler</li> <li>{@link txtfnnl.tika.sax.ElsevierXMLContentHandler}
   * for XML in Elsevier's DTD format</li> <li>{@link txtfnnl.tika.sax.PubMedCentralXMLContentHandler}
   * fro XML in NLM's archive DTD format</li> <li>{@link txtfnnl.tika.sax.CleanBodyContentHandler}
   * only removes "ignorable whitespaces" and uses the default Tika XML parser</li> </ul> Use the
   * Elsevier-specific XML handler instead of the default handler.
   */
  ...
  //TikaWrapper tika = new TikaWrapper(); // TODO: remove this
  ...
  class Builder extends AnalysisComponentBuilder {
    ...
    public
    Builder setXmlHandlerClass(Class<? extends ContentHandler> klass) {
      setOptionalParameter(PARAM_XML_HANDLER, (klass == null) ? null : klass.getName());
      return this;
    }

    public
    Builder normalizeGreek() {
  ...
  /**
   * The AE process expects a SOFA with a {@link Views#CONTENT_RAW} and uses Tika to produce a new
   * plain-text SOFA with a {@link Views#CONTENT_TEXT}. It preserves all metadata annotations on the
  ...
  void process(JCas aJCas) throws AnalysisEngineProcessException {
    ...
    if (aJCas.getSofaDataURI() != null) {
      try {
        metadata.set(
            TikaMetadataKeys.RESOURCE_NAME_KEY, resourceName(new URI(aJCas.getSofaDataURI()))
        );
    ...
    try {
      ...
      if ("text/html".equals(mediaType) || mediaType.startsWith("application/xhtml")) {
        ...
        if (XMLContentHandler.class.getName().equals(xmlHandlerClass)) {
          ...
          parser = new UnembeddedXMLParser();
        } else if (PubMedCentralXMLContentHandler.class.getName().equals(xmlHandlerClass)) {
          handler = new PubMedCentralXMLContentHandler(handler);
          parser = new UnembeddedXMLParser();
  ...
  private
  String resourceName(URI uri) throws MalformedURLException {
