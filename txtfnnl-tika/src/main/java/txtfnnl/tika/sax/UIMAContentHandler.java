...
public class UIMAContentHandler extends ContentHandlerDecorator {
     ...
     * identifier, annotator and property annotations. The confidence of the annotations will
     * always be 1.
     */
    @Override
    public void startElement(String uri, String lName, String qName, Attributes atts) {
        ...
        if (num_atts > 0) {
            ...
            for (int idx = 0; idx < num_atts; ++idx) {
                ...
                final String pName =
                    UIMAContentHandler.chooseName(atts.getLocalName(idx), atts.getQName(idx));
                prop.setName(pName);
    ...
    /**
     * Set the end of the first matching, started element to offset in the text being extracted
     * (into a buffer) and add the finished annotation to the indexes of the CAS view.
     * 
