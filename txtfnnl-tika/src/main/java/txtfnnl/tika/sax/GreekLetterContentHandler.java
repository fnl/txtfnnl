...
/**
 * A content handler decorator that translates Greek letters to their
 * spelled-out, Latin names.
 * 
 * Upper-case letters are written capitalized (Alpha, Beta, ...), while
 * lower-case letters are written all small letters (alpha, beta, ...).
 * 
...
public class GreekLetterContentHandler extends ContentHandlerDecorator {
	...
	/**
	 * Normalize Greek characters to their spelled-out Latin (ASCII) letter
	 * form.
	 */
	@Override
	public void characters(char ch[], int offset, int len) throws SAXException {
		if (len > 0) {
			...
			for (int pos = offset; pos < end; ++pos) {
				for (idx = GREEK_CHARS.length; idx-- > 0;) {
					if (GREEK_CHARS[idx] == ch[pos]) {
						...
						if (cleanLen > 0)
							super.characters(ch, offset, cleanLen);
						
						super.characters(GREEK_NAMES[idx], 0,
						    GREEK_NAMES[idx].length);
						offset = pos + 1;
						len -= cleanLen + 1;
						break;
					}
			...
			if (len > 0)
				super.characters(ch, offset, len);
		}
	}
}
