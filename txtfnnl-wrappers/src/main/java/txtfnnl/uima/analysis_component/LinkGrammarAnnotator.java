...
public class LinkGrammarAnnotator extends JCasAnnotator_ImplBase {
     ...
     * <p>
     * Unless the link-parser changed its output representation (and you are getting
     * RuntimeErrors), you should not have to be interested in this class.
     * 
     * @author Florian Leitner
     */
    static class ConstituentNode implements Iterable<ConstituentNode> {
        ...
        private Offset span;
        private static final LinkedList<ConstituentNode> EMPTY_LIST =
            new LinkedList<ConstituentNode>();
        ...
        /**
         * Parse all contained expression to child nodes of the given parent, returning the offset
         * up to which the expression could be parsed.
         * 
        ...
        public static Iterator<ConstituentNode> walk(final ConstituentNode node) {
            return new Iterator<ConstituentNode>() {
                private final Stack<Iterator<ConstituentNode>> stack =
                    new Stack<Iterator<ConstituentNode>>();
     ...
     * ), and the language is English, it is not necessary to set this parameter. I.e., by default,
     * the English dictionaries are used. Therefore, the default value of this parameter is
     ...
     * @throws IOException
     */
    public static AnalysisEngineDescription configure(int timeout) throws UIMAException,
            IOException {
    ...
    public void process(JCas jcas) throws AnalysisEngineProcessException {
        ...
        while (sentenceIt.hasNext()) {
            ...
            phrases.addAll(parseSentence(sentenceAnn.getCoveredText(), sentenceAnn.getBegin(),
                textCas));
        }
        ...
        if (constituentExpression != null) {
            ...
            for (final char chop : new char[] { ';', ',' }) {
                ...
                while ((idx = sentence.indexOf(chop, idx + 1)) != -1) {
                    phrases.addAll(parseSentence(sentence.substring(last, idx), offset + last,
                        jcas));
                    last = idx + 1;
                }
                if (last != 0) {
                ...
                if (lastOffset != null && n.getOffset().equals(lastOffset)) {
                    ...
                    logger.log(Level.FINE,
                        "dropping outer constituent {0} of {1} on span ''{2}''", new Object[] {
                            ann.getIdentifier(), n.data, ann.getCoveredText() });
