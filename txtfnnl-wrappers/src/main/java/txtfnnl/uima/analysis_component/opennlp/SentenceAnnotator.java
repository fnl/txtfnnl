...
public final class SentenceAnnotator extends JCasAnnotator_ImplBase {
     ...
     * 
     * @param modelFile containing the OpenNLP sentence segmentation model to use; this parameter
     *        is <b>required</b>
    ...
    public static AnalysisEngineDescription configure(File modelFile, String splitSentences)
        ...
        if (splitSentences == null) {
            ...
            aed =
                AnalysisEngineFactory.createPrimitiveDescription(SentenceAnnotator.class,
    ...
    public void initialize(UimaContext ctx) throws ResourceInitializationException {
        ...
        try {
            final SentenceModelResource modelResource =
                (SentenceModelResource) ctx.getResourceObject(RESOURCE_SENTENCE_MODEL);
        ...
        for (final String cnk : chunks) {
            ...
            for (int i = 0; i < spans.length; i++) {
                final SentenceAnnotation sentence =
                    new SentenceAnnotation(jcas, offset + spans[i].getStart(), offset +
                        spans[i].getEnd());
