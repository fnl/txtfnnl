...
public final class RelationshipPatternLineWriter extends TextWriter {
    ...
    public static final String PARAM_RELATIONSHIP_NAMESPACE =
    ...
    public static FSMatchConstraint makeConstituentSyntaxAnnotationConstraint(JCas jcas,
        ...
        final Feature constituentNamespaceFeat =
            jcas.getTypeSystem().getFeatureByFullName(
    ...
    /**
     * Configure an CAS consumer descriptor for a pipeline. Note that if the
     * {@link #outputDirectory} is <code>null</code> and {@link #printToStdout} is
     * <code>false</code>, a {@link ResourceInitializationException} will occur when creating the
     * AE.
     * 
    ...
    public static AnalysisEngineDescription configure(final String relationshipNamespace,
            ...
            final boolean overwriteFiles, final int maxPatternLength) throws UIMAException,
            IOException {
    ...
    public static AnalysisEngineDescription configure(String relationshipNamespace)
        ...
        return RelationshipPatternLineWriter.configure(relationshipNamespace, null, null, true,
            false, 0);
    }
    ...
    public void process(CAS cas) throws AnalysisEngineProcessException {
        ...
        final FSIterator<TOP> relationshipIt =
            KnownRelationshipAnnotator.getRelationshipIterator(textJCas, relationshipNamespace);
        final String text = textJCas.getDocumentText();
        final AnnotationIndex<Annotation> annIdx =
            textJCas.getAnnotationIndex(SyntaxAnnotation.type);
        final FSMatchConstraint constituentConstraint =
            RelationshipPatternLineWriter.makeConstituentSyntaxAnnotationConstraint(textJCas);
    ...
    void process(RelationshipAnnotation relAnn, String text, AnnotationIndex<Annotation> annIdx,
        ...
        final LinkedList<Annotation> nounPhrases =
            extractNounPhrases(jcas.createFilteredIterator(
                annIdx.subiterator(sentAnn, true, true), constituentConstraint));
        final FSIterator<Annotation> constituentIt =
            jcas.createFilteredIterator(annIdx.subiterator(sentAnn, true, true),
                constituentConstraint);
        final List<List<Annotation>> constituentCombinations =
            listConstituentArrangements(constituentIt);
        if (logger != null) {
        ...
        for (final TextAnnotation[] entities : listSeparateEntities(relAnn.getTargets())) {
            ...
            final List<TextAnnotation[]> entityPermutations =
                combine(findNPReplacements(entities, nounPhrases));
    ...
    /**
     * Create all possible constituent arrangements by skipping PP, ADJP, ADVP, SBAR, SBARQ, and
     * WHNP spans or only creating arrangements containing inner sentences (S, SBAR, SBARQ, SQ,
     * SINV).
     * 
     ...
     * has a partial overlap any other. For each annotated entity type (NS-ID-based), only one
     * entity mention of that type is used to form part of a valid annotation group, even if that
     * entity type is annotated multiple times on the same sentence. All valid combinations of such
     * groups based on the input array are returned. In each group, the entities are ordered by
     * increasing offsets.
     * 
    ...
    /**
     * Extract a set of patterns from the annotations that condenses any NP that contain an entity
     * to make the NP represent that entity only.
     * 
     * @param entities
     * @param spans
     * @param nounPhrases
    ...
    /**
     * Create all possible combinations from a list of replacement sets at each possible position
     * in the array using recursive calls of this method.
     * 
    ...
    /**
     * Sort entities by increasing offsets (increasing start, decresing end) and all other
     * comparator settings (see {@link TextAnnotation#TEXT_ANNOTATION_COMPARATOR}).
     * 
     * @param entities
    ...
    /**
     * Add a space character if the string builder contains characters and the last character is
    ...
    /**
     * Remove multiple spaces, replace single line breaks, and escape HTML from a substring within
     * a text.
     * 
    ...
    private String replaceEmptyModifierPhrases(String text) {
        String replacement =
            REGEX_EMPTY_MODIFIER.matcher(text).replaceAll(" ").replace(" , ", " ").trim();
