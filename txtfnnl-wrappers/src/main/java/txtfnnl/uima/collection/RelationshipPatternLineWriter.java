...
public final class RelationshipPatternLineWriter extends TextWriter {
  ...
  /**
   * Define the maximum number of characters a pattern may contain. If unset, all lengths are
   * printed, which is the same as setting this parameter to 0.
   */
  ...
  public static FSMatchConstraint makeConstituentSyntaxAnnotationConstraint(JCas jcas,
      String constituentNamespace) {
    final Feature constituentNamespaceFeat = jcas.getTypeSystem().getFeatureByFullName(
        SyntaxAnnotation.class.getName() + ":namespace");
    final ConstraintFactory cf = jcas.getConstraintFactory();
    final FeaturePath namespacePath = jcas.createFeaturePath();
    namespacePath.addFeature(constituentNamespaceFeat);
    final FSStringConstraint namespaceCons = cf.createStringConstraint();
    namespaceCons.equals(constituentNamespace);
    return cf.embedConstraint(namespacePath, namespaceCons);
  }

  /**
   * Make a constraint that allows iterating constituent annotations in a CAS. Defaults to using
   * the LinkGrammarAnnotator's {@link LinkGrammarAnnotator#NAMESPACE namespace}.
   */
  public static FSMatchConstraint makeConstituentSyntaxAnnotationConstraint(JCas jcas) {
    return RelationshipPatternLineWriter.makeConstituentSyntaxAnnotationConstraint(jcas,
        LinkGrammarAnnotator.NAMESPACE);
  }

  /**
   * Configure an CAS consumer descriptor for a pipeline. Note that if the {@link #outputDirectory}
   ...
   * @param maxPatternLength the max length of patterns to extract (or 0 for unlimited)
  ...
  public void process(CAS cas) throws AnalysisEngineProcessException {
    ...
    final FSIterator<TOP> relationshipIt = KnownRelationshipAnnotator.getRelationshipIterator(
        textJCas, relationshipNamespace);
    final String text = textJCas.getDocumentText();
    final AnnotationIndex<Annotation> annIdx = textJCas.getAnnotationIndex(SyntaxAnnotation.type);
    final FSMatchConstraint constituentConstraint = RelationshipPatternLineWriter
        .makeConstituentSyntaxAnnotationConstraint(textJCas);
    logger.log(Level.FINE, "processing document '" + documentId + "'");
    while (relationshipIt.hasNext()) {
      process((RelationshipAnnotation) relationshipIt.next(), text, annIdx, constituentConstraint,
          textJCas);
    }
    try {
      unsetStream();
  ...
  void process(RelationshipAnnotation relAnn, String text, AnnotationIndex<Annotation> annIdx,
      FSMatchConstraint constituentConstraint, JCas jcas) throws AnalysisEngineProcessException {
    final SentenceAnnotation sentAnn = (SentenceAnnotation) relAnn.getSources(0);
    final LinkedList<Annotation> nounPhrases = extractNounPhrases(jcas.createFilteredIterator(
        annIdx.subiterator(sentAnn, true, true), constituentConstraint));
    final FSIterator<Annotation> constituentIt = jcas.createFilteredIterator(
        annIdx.subiterator(sentAnn, true, true), constituentConstraint);
    final List<List<Annotation>> constituentCombinations = listConstituentArrangements(constituentIt);
    if (logger != null) {
      logger.log(Level.FINER, "processing sentence '" + sentAnn.getCoveredText() + "'");
    }
    for (final TextAnnotation[] entities : listSeparateEntities(relAnn.getTargets())) {
      final Set<String> patterns = new HashSet<String>();
      final LinkedList<Annotation> sentenceSpan = new LinkedList<Annotation>();
      final List<TextAnnotation[]> entityPermutations = combine(findNPReplacements(entities,
          nounPhrases));
      ...
      for (final List<Annotation> spans : constituentCombinations) {
      ...
      if (logger != null && logger.isLoggable(Level.FINER) && entities.length > 0) {
        ...
        for (final TextAnnotation e : entities) {
          sb.append('"');
          sb.append(e.getCoveredText());
          sb.append("\", ");
        }
        sb.delete(sb.length() - 2, sb.length());
        logger.log(Level.FINER, "the entities: " + sb.toString() + " have " + patterns.size() +
            " patterns");
      }
      try {
  ...
  LinkedList<Annotation> extractNounPhrases(FSIterator<Annotation> constituents) {
    final LinkedList<Annotation> nps = new LinkedList<Annotation>();
    while (constituents.hasNext()) {
      final SyntaxAnnotation ann = (SyntaxAnnotation) constituents.next();
  ...
  /**
   * Create all possible constituent arrangements by skipping PP, ADJP, ADVP, SBAR, SBARQ, and WHNP
   * spans or only creating arrangements containing inner sentences (S, SBAR, SBARQ, SQ, SINV).
   * 
  ...
  List<List<Annotation>> listConstituentArrangements(FSIterator<Annotation> constituentIt) {
    final List<List<Annotation>> arrangements = new ArrayList<List<Annotation>>();
    ...
    while (constituentIt.hasNext()) {
      ...
      if (mayBeSkipped(ann)) {
        for (int i = arrangements.size(); i-- > 0;) {
          final List<Annotation> spans = arrangements.get(i);
          final SyntaxAnnotation last = (SyntaxAnnotation) spans.get(spans.size() - 1);
          if (last.contains(ann)) {
            final List<Annotation> clone = new LinkedList<Annotation>(spans);
            clone.remove(clone.size() - 1);
            arrangements.add(clone);
            if (last.getBegin() < ann.getBegin()) {
              final Annotation segment = (Annotation) last.clone();
            ...
            if (last.getEnd() > ann.getEnd()) {
              ...
              final Annotation dummy = (Annotation) last.clone();
    ...
    for (final List<Annotation> list : arrangements) {
  ...
  /**
   * Return all valid combinations of entity annotations. In a valid annotation group, no entity
   * has a partial overlap any other. For each annotated entity type (NS-ID-based), only one entity
   * mention of that type is used to form part of a valid annotation group, even if that entity
   * type is annotated multiple times on the same sentence. All valid combinations of such groups
   * based on the input array are returned. In each group, the entities are ordered by increasing
   * offsets.
   * 
   * @param entityArray of TextAnnotations of the entities
   * @return a List of all valid annotation groups, ordered by increasing offsets
   */
  List<TextAnnotation[]> listSeparateEntities(FSArray entityArray) {
    final List<TextAnnotation[]> list = new ArrayList<TextAnnotation[]>();
    final Map<String, List<TextAnnotation>> entityMap = groupEntities(entityArray);
    final int choices = entityMap.size();
    if (choices > 0) {
      // create all possible combinations of entities choosing one from
      // each type
      int choice = 0;
      for (final List<TextAnnotation> entities : entityMap.values()) {
        final int numEntities = entities.size();
        if (choice == 0) {
          for (final TextAnnotation e : entities) {
            final TextAnnotation[] anns = new TextAnnotation[choices];
            anns[choice] = e;
            list.add(anns);
          }
        } else {
          ...
          for (int i = 1; i < numEntities; i++) {
            ...
            for (int j = 0; j < size; j++) {
              final TextAnnotation[] anns = list.get(j);
              final TextAnnotation[] copy = Arrays.copyOf(anns, anns.length);
              copy[choice] = e;
              list.add(copy);
            }
          }
    ...
    final Iterator<TextAnnotation[]> it = list.iterator();
    while (it.hasNext()) {
  ...
  /**
   * Return a list of Annotation sets for each entity, where the inner sets contain the entity
   * annotation and all noun phrases that may be used to replace it.
   * 
   * @param entities
   * @param nounPhrases
  ...
  List<Set<Annotation>> findNPReplacements(TextAnnotation[] entities,
    ...
    final List<Set<Annotation>> replacements = new ArrayList<Set<Annotation>>(entities.length);
    int eb, ee;
    // collect all possible NP replacements for the entities plus the
    // entity itself
    for (final TextAnnotation e : entities) {
      final Set<Annotation> inner = new HashSet<Annotation>();
      inner.add(e);
      eb = e.getBegin();
      ee = e.getEnd();
      for (final Annotation np : nounPhrases) {
        if (np.getBegin() <= eb && np.getEnd() >= ee && (np.getBegin() != eb || np.getEnd() != ee)) {
          final TextAnnotation clone = (TextAnnotation) e.clone();
  ...
  String extractPattern(TextAnnotation[] entities, List<Annotation> spans)
    ...
    final StringBuilder sb = new StringBuilder();
    for (final Annotation a : spans) {
      final String text = a.getCoveredText();
  ...
  /**
   * Extract a set of patterns from the annotations that condenses any NP that contain an entity to
   * make the NP represent that entity only.
   * 
   * @param entities
   * @param spans
   * @param nounPhrases
   * @return a set of patterns that condense one or more of the NPs containing the entities
   * @throws AnalysisEngineProcessException
   */
  Set<String> extractNounPhraseSkippedPatterns(List<Annotation> spans,
      List<TextAnnotation[]> permutations) throws AnalysisEngineProcessException {
    final Set<String> patterns = new HashSet<String>();
    // iterate over all possible entity permutations
    for (final TextAnnotation[] permutation : permutations) {
  ...
  private void startNewSpans(SyntaxAnnotation ann, List<List<Annotation>> arrangements) {
    final List<Annotation> list = new LinkedList<Annotation>();
    list.add(ann);
  ...
  private boolean containsAllEntities(List<Annotation> spans, TextAnnotation[] entities) {
    int idx = 0;
    for (final Annotation ann : spans) {
      while (idx < entities.length && ann.getBegin() <= entities[idx].getBegin() &&
  ...
  /**
   * Helper method to expand the arrangements with a cloned version of spans where the current
   * annotation is skipped and is added to spans. Furthermore, if the last annotation is a dummy
   * (begin == end), it is removed from both spans and the clone.
   * 
   * @param spans
  ...
  private void expandWithAndWithout(List<Annotation> spans, SyntaxAnnotation currentAnnotation,
      SyntaxAnnotation lastAnnotation, List<List<Annotation>> arrangements)
      throws CASRuntimeException {
    if (lastAnnotation.getBegin() == lastAnnotation.getEnd()) {
      spans.remove(spans.size() - 1);
    }
    final List<Annotation> clone = new LinkedList<Annotation>(spans);
    final Annotation dummy = (Annotation) currentAnnotation.clone();
    dummy.setBegin(dummy.getEnd());
    clone.add(dummy);
    arrangements.add(clone);
    spans.add(currentAnnotation);
    if (spans.size() == 0) throw new AssertionError("empty spans");
    if (clone.size() == 0) throw new AssertionError("empty clone");
  }

  /**
   * Create all possible combinations from a list of replacement sets at each possible position in
   * the array using recursive calls of this method.
   * 
  ...
  private List<TextAnnotation[]> combine(List<Set<Annotation>> replacements) {
    final List<List<Annotation>> combinations = SetUtils.combinate(replacements);
    final List<TextAnnotation[]> result = new ArrayList<TextAnnotation[]>(combinations.size());
    final int len = replacements.size();
    for (final List<Annotation> list : combinations) {
  ...
  Map<String, List<TextAnnotation>> groupEntities(FSArray entities) {
    final Map<String, List<TextAnnotation>> map = new HashMap<String, List<TextAnnotation>>();
    if (entities != null) {
      for (int i = entities.size(); i-- > 0;) {
        final TextAnnotation ann = (TextAnnotation) entities.get(i);
        final String key = ann.getNamespace() + ann.getIdentifier();
  ...
  /**
   * Sort entities by increasing offsets (increasing start, decresing end) and all other comparator
   * settings (see {@link TextAnnotation#TEXT_ANNOTATION_COMPARATOR}).
   * 
   * @param entities
   * @return the same array or <code>null</code> if partial overlaps were detected in the array
   */
  TextAnnotation[] orderEntities(TextAnnotation[] entities) {
    if (entities != null) {
      ...
      for (final TextAnnotation e : entities) {
        if (last != null && e.getBegin() < last.getEnd() && e.getEnd() > last.getEnd()) // partially
                                                                                        // overlapping
                                                                                        // entities
                                                                                        // detected
          return null;
  ...
  /**
   * Add a space character if the string builder contains characters and the last character is not
   * a space character.
   * 
   * @param sb
   */
  ...
  /**
   * Remove multiple spaces, replace single line breaks, and escape HTML from a substring within a
   * text.
   * 
  ...
  /**
   * After all this trimming, sometimes empty phrases might lead to dangling commas and dots that
   * should be replaced.
   * 
  ...
  private String replaceEmptyModifierPhrases(String text) {
    String replacement = REGEX_EMPTY_MODIFIER.matcher(text).replaceAll(" ").replace(" , ", " ")
        .trim();
  ...
  private String replaceSingleBreaksAndSpaces(String text) {
    return REGEX_SINGLE_LINEBREAK.matcher(REGEX_LINEBREAK_SPACE.matcher(text).replaceAll("$1"))
        .replaceAll(" ");
  }
}
