...
public class TestGnamedGazetteerAnnotator {
  ...
  @Test
  public void testGreekMapping() throws SQLException, UIMAException {
    ExternalResourceDescription gazetteer = GnamedGazetteerResource.configure(url, "org.h2.Driver", query).boundaryMatch().idMatching().create();
    AnalysisEngineDescription descriptor = GazetteerAnnotator.configure("nsX", gazetteer).create();
    ...
    final JCas jcas = makeJCas(ae, "that has IL-1\u03B2 and HPRT but not HPRTJUNK in it");
    ae.process(jcas);
    ...
    while (it.hasNext()) {
      ...
      assertEquals("nsX", ann.getNamespace());
      if (count == 0) {
  ...
  public void testExpansion() throws SQLException, UIMAException {
    ExternalResourceDescription gazetteer = GnamedGazetteerResource.configure(url, "org.h2.Driver", query).create();
    ...
    while (it.hasNext()) {
      ...
      if (ann.getOffset().start() == 1) {
        if (ann.getOffset().end() == 6) {
          ...
          if (!("1".equals(ann.getIdentifier()) || "2".equals(ann.getIdentifier())))
            fail(ann.toString());
        } else {
          fail(ann.toString());
        }
