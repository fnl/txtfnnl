...
public class XmiWriter extends CasConsumer_ImplBase {
    /**
     * Required configuration parameter String that defines the path to a directory where the
     * output files will be written. Note that the directory will be created if it does not exist.
     */
     ...
     * file unique, the existing file is replaced (where <i>n</i> is some integer that would make
    ...
    /**
     * Consume a CAS to produce a file in the XML Metadata Interchange format. This consumer
     * expects the CAS to have both a raw and a text view. The file URI is fetched from the raw
     * view, while the XMI content is created from the text view.
     * 
