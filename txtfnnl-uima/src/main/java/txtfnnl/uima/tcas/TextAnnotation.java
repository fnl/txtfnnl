...
/**
 * Annotations on a specific text span. Updated by JCasGen Tue Nov 27 14:20:54
 * CET 2012 XML source:
...
public class TextAnnotation extends Annotation {
	 ...
	 */
	protected static FSMatchConstraint makeConstraint(String typeName,
	                                                  JCas jcas,
	                                                  String annotatorUri,
	                                                  String namespace,
	                                                  String identifier) {
		...
		if (annotatorUri != null) {
			constraint = createConstraint(typeName + ":annotator", jcas,
			    annotatorUri, cf);
		}

		if (namespace != null) {
			if (constraint == null)
				constraint = createConstraint(typeName + ":namespace", jcas,
				    namespace, cf);
			else
				constraint = cf.and(
				    constraint,
				    createConstraint(typeName + ":namespace", jcas, namespace,
				        cf));
		...
		if (identifier != null) {
			if (constraint == null)
				constraint = createConstraint(typeName + ":identifier", jcas,
				    identifier, cf);
			else
				constraint = cf.and(
				    constraint,
				    createConstraint(typeName + ":identifier", jcas,
				        identifier, cf));
	...
	private static FSMatchConstraint createConstraint(String featureName,
	                                                  JCas jcas,
		...
		FSStringConstraint fsStringConstraint = cf.createStringConstraint();
	...
	public static FSMatchConstraint makeConstraint(JCas jcas,
	                                               ...
	                                               String namespace,
	                                               String identifier) {
		return TextAnnotation.makeConstraint(TextAnnotation.class.getName(),
