...
public abstract class KnownEvidenceAnnotator<Evidence> extends JCasAnnotator_ImplBase {
    ...
    public void process(JCas jcas) throws AnalysisEngineProcessException {
        ...
        if (evidence == null) {
            logger.log(Level.WARNING, "no evidence mapped to doc {0} ({1})", new Object[] {
                documentId, rawCas.getSofaDataURI() });
        } else {
     ...
     * the CAS. In addition, the counter vars truePositives, falsePositivies, and checksum should
     * be incremented are appropriate (checksum should be the same as the sum of TPs and FPs).
     * 
