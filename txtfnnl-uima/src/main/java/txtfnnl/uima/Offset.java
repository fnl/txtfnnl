...
/**
 * A data structure to manage comparable offset values.
 * 
 * Offset values are lists of integers with length 1 or 2n where n > 0. An
 * Offset is immutable. Two Offsets with exactly the same values are
...
public class Offset implements Comparable<Offset> {
	 ...
	 * Create a new Offset.
	 * 
	 * Offset values always have to be increasing from one value to the next.
	 * 
	 * @param values one or 2n offset values
	 * @throws IllegalArgumentException if the wrong number of values is given
	 *         or if the values are not incremental
	 */
	public Offset(int... values) {
		if (values.length == 0)
			throw new IllegalArgumentException("no offset values");
	...
	/**
	 * Compare this Offset to another.
	 * 
	 * An offset that starts before another is considered smaller, while with
	 * equal starts, the one with the larger end value is considered smaller
	 * for ordering. If the starts and ends are equal, the Offset with the
	 * first value that is larger than the other's is ordered first.
	 * 
	 * @param other offset to compare
	 * @return -1 if this Offset is smaller than the other, 1 if larger, and 0
	 *         if they are equal.
	 */
	 ...
	 * @return true if all spans (and, if present, sub-spans) of the other
	 *         offset are contained exactly within this offset
	 */
	public boolean contains(Offset other) {
		int oLen = other.size();

		if (!contains(start(), end(), other.start(), other.end())) {
			return false; // impossible that the other is within this Offset
		} else if (values.length == 2 && oLen < 3) {
			return true; // other must have length 1 or 2, within this Offset
		} else if (values.length == 1) {
			return true; // other must be an Offset at the same position
		} else {
			...
			Set<Offset> matches = new HashSet<Offset>();
			List<Offset> tests = new LinkedList<Offset>();
			int[] other_values = other.values();
			int idx, start, end;
			...
			if (oLen < 3) {
				...
				for (idx = oLen / 2; idx-- > 0;) {
					tests.add(new Offset(other_values[idx * 2],
					    other_values[idx * 2 + 1]));
				}
			}
			for (idx = values.length / 2; idx-- > 0;) {
				...
				for (Offset m : tests) {
					if (contains(start, end, m.start(), m.end()))
						matches.add(m);
				}
			}
