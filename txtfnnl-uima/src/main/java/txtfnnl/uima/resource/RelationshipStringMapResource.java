 ...
 * always have <code>1 + 3n</code> items. If there
 * is more than one relationship associated to a key string, multiple lines
 ...
 */
public class RelationshipStringMapResource extends
        LineBasedStringMapResource<List<Set<Entity>>> {

	@Override
	void parse(String[] items) throws ResourceInitializationException {
		if ((items.length - 1) % 3 != 0)
			new ResourceInitializationException(new RuntimeException(
		...
		for (int idx = 0; idx < numEntities; ++idx)
			entities.add(new Entity(items[3 * idx + 1], items[3 * idx + 2],
			    items[3 * idx + 3]));
