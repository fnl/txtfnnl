...
/**
 * Annotate known entities, generating the annotations in UIMA XMI format.
 * 
 * The entities are "known", because for each file, the IDs of the contained
 * entities must be provided. These IDs are used to fetch their actual names
 * from a DB resource, and finally, a regular expression is used to detect the
 * presence of those names in the text.
 * 
...
public class EntityMentionAnnotator {
	...
	public static void main(String[] arguments) {
		...
		Logger l = Pipeline
		    .loggingSetup(cmd, opts, "txtfnnl ema [options] <directory|files...>\n");
		...
		try {
			jdbcResource = Pipeline.getJdbcResource(cmd, l, DEFAULT_JDBC_DRIVER,
		...
		String entityMapPath = cmd.getOptionValue('m', DEFAULT_MAPPING_FILE);
		...
		try {
			Pipeline pipeline = new Pipeline(2); // tika and entity detector
			pipeline.setReader(cmd);
			pipeline.configureTika(cmd);
			pipeline.set(1,
			    KnownEntityAnnotator.configure(namespace, queries, entityMap, jdbcResource));
			pipeline.setConsumer(XmiWriter.configure(outputDirectory, encoding, overwriteFiles));
			pipeline.run();
