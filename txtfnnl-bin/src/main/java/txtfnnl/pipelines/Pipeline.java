 ...
 * 
 * In the simplest case (only extracts the plain text), a pipeline can be set
 * up as follows:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * This will generate a simple pipeline that reads all files in the CWD
 * ("user.dir"), extracts their content with Tika, and writes that content to
 * STDOUT.
 * 
...
public class Pipeline {
	...
	/**
	 * Add default command-line options for any pipeline.
	 * 
	...
	/**
	 * Add standard output options for any pipeline.
	 * 
	 * The added options are:
	 * <ul>
	...
	public static void addOutputOptions(Options opts) {
		opts.addOption("E", "output-encoding", true,
		    "use encoding [" + (IOUtils.isMacOSX() ? "UTF-8" : Charset.defaultCharset()) + "]");
	...
	/**
	 * Add Tika command-line options for any pipeline.
	 * 
	 * The added options are
	 * <ul>
	...
	/**
	 * Add the command-line options to use the JdbcConnectionResource for any
	 * pipeline.
	 * 
	 * The added options are
	 * <ul>
	...
	/**
	 * Set up logging unless the help option (h) is set.
	 * 
	 * If the help option is set, print the help and exit (
	...
	public static Logger loggingSetup(CommandLine cmd, Options opts, String usage) {
		...
		Logger l = Logger.getLogger(Pipeline.class.getName());
	...
	/**
	 * Ensure a readable output directory or return <code>null</code> if
	 * unset.
	 * 
	 * * Calls {@link System#exit(int)} if the option isn't <code>null</code>
	 * or a writeable directory.
	 * 
	 */
	public static File outputDirectory(CommandLine cmd) {
		...
		if (cmd.hasOption('o')) {
			...
			if (!outputDirectory.isDirectory() || !outputDirectory.canWrite()) {
				System.err
				    .println("cannot write to directory '" + outputDirectory.getPath() + "'");
				System.exit(1); // == exit ==
			}
		}
	...
	/**
	 * Get the right {@link Pipeline.XmlHandler} given the option value.
	 * 
	 * Calls {@link System#exit(int)} if an unknown handler was set.
	 * 
	...
	public static XmlHandler getTikaXmlHandler(CommandLine cmd) {
		String handler = cmd.getOptionValue('x', "default");

		if ("default".equals(handler))
			return XmlHandler.DEFAULT;
		else if ("clean".equals(handler))
			return XmlHandler.CLEAN;
		else if ("elsevier".equals(handler))
			return XmlHandler.ELSEVIER;

		System.err.println("unknown XML hanlder '" + handler + "'");
		System.exit(1); // == exit ==
		return null;
	}
	 ...
	 * @param defaultProvider the default DB provider (for that driver class)
	 *        if not given
	...
	public static ExternalResourceDescription getJdbcResource(CommandLine cmd, Logger l,
		...
		String driverClass = cmd.getOptionValue('D', defaultDriverClass);
		String dbHost = cmd.getOptionValue('H', "localhost");
		String dbProvider = cmd.getOptionValue('P', defaultProvider);
		String dbName = cmd.getOptionValue('d', defaultDbName);
		String dbPassword = cmd.getOptionValue('p');
		String dbUsername = cmd.getOptionValue('u');
		String dbUrl; // (from: P, s, d)

		Class.forName(driverClass);
		dbUrl = "jdbc:" + dbProvider + "://" + dbHost + "/" + dbName;
		l.info("JDBC URL: " + dbUrl);
		return JdbcConnectionResourceImpl.configure(dbUrl, driverClass, dbUsername, dbPassword,
		    true);
	}

	/**
	 * Configure a pipeline with a given reader and the number of engines it
	 * will have (without counting the CAS consumer).
	 * 
	 * The actual engines must be set at a later stage.
	 * 
	 * @param reader a collection reader configuration
	 * @param numEngines number of analysis engines the pipeline will consist
	 *        of without last "engine" that should be a CAS consumer.
	 */
	public Pipeline(CollectionReaderDescription reader, int numEngines) {
		collectionReader = reader;

		if (numEngines < 0)
			throw new IllegalArgumentException("numEngines=" + numEngines);

		pipeline = new AnalysisEngineDescription[numEngines + 1];
	}

	/**
	 * Configure a pipeline with the number of engines it will have (without
	 * counting the CAS consumer).
	 * 
	 * The collection reader and actual engines must be set at a later stage.
	 * 
	 * @param numEngines number of analysis engines the pipeline will consist
	 *        of without last "engine" that should be a CAS consumer.
	 */
	...
	/**
	 * Configure a (default) pipeline with one engine (without counting the
	 * CAS consumer).
	 * 
	 * The collection reader, engine, and consumer must be set at a later
	 * stage.
	 */
	public Pipeline() {
		this(null, 1);
	...
	/**
	 * Configure a pipeline with a reader for one analysis engines (without
	 * counting the CAS consumer).
	 * 
	 * The all but the collection reader must be set at a later stage.
	 * 
	 * @param engines the analysis engines the pipeline uses, including the
	 *        final CAS consumer
	 */
	...
	/**
	 * Configure a pipeline with a given reader and its analysis engines.
	 * 
	 * @param reader a collection reader configuration
	 * @param engines the analysis engines the pipeline uses, including the
	 *        final CAS consumer
	...
	/**
	 * Configure a pipeline with its analysis engines.
	 * 
	 * The collection reader must be set at a later stage.
	 * 
	 * @param engines the analysis engines the pipeline uses, including the
	 *        final CAS consumer
	 */
	...
	public CollectionReaderDescription setReader(CollectionReaderDescription reader) {
		CollectionReaderDescription last = collectionReader;
		collectionReader = reader;
		return last;
	}

	/**
	 * Set up a collection reader for this pipeline according to the given
	 ...
	 */
	public CollectionReaderDescription setReader(CommandLine cmd) throws IOException,
	        UIMAException {
		...
		if (inputFiles.length > 0) {
			if (inputFiles.length == 1) {
				...
				if (inputDirectory.isFile() && inputDirectory.canRead()) {
					...
					System.err.println("path '" + inputFiles[0] + "' not a (readable) directory");
					throw new IOException("path '" + inputFiles[0] +
				...
				for (String fn : inputFiles) {
					...
					if (!tmp.canRead() || !tmp.isFile()) {
						System.err.println("path '" + fn + "' not a (readable) file");
						throw new IOException("path '" + fn + "' not a (readable) file");
					}
				}
			}
		} else {
		...
		if (inputDirectory == null)
			return setReader(inputFiles, mimeType);
		else
			return setReader(inputDirectory, mimeType, recursive);
	}

	/**
	 * Set a file collection reader for this pipeline.
	 * 
	 ...
	 * Set a file collection reader for this pipeline.
	 * 
	 * Detects the MIME type using Tika.
	 * 
	 * @param inputFiles the input file (paths) to read
	 * @return a formerly configured collection reader (if any)
	 * @throws IOException
	...
	/**
	 * Set a directory collection reader for this pipeline.
	 * 
	...
	/**
	 * Set a <i>non-recursive</i> directory collection reader for this
	 * pipeline.
	 * 
	 * @param inputDirectory where the input files are located
	 * @param mimeType the MIMIE type of the input files
	...
	public AnalysisEngineDescription setConsumer(AnalysisEngineDescription consumer) {
		AnalysisEngineDescription before = pipeline[size()];
	 ...
	 * 
	 * Note that the pipeline must have at least two analysis engines (the
	 * Tika extractor and a CAS consumer) and thus the idx position may not be
	 * the last AE in the pipeline (which should be set to a CAS consumer).
	 * 
	 * @param idx the position in the pipeline the AE should have (where 0 is
	 *        the first AE after the reader)
	 ...
	 * @param encoding optional character encoding to use (otherwise the
	 *        platform default is used); may be <code>null</code>
	 * @param normalizeGreek if <code>true</code>, Greek characters (including
	 *        the letter "sharp S" (U+00DF) - that will be treated as "beta")
	 *        will be normzalized to spelled-out ASCII (i.e., using Latin
	 *        letters).
	...
	public AnalysisEngineDescription configureTika(int idx, boolean simple, String encoding,
	                                               boolean normalizeGreek, XmlHandler handler)
	        throws UIMAException, IOException {
		if (pipeline.length < 2)
			throw new IllegalStateException(
			    "trying to configure a Tika AE on a pipeline of length 1");
		...
		String xmlHandlerClass = XMLContentHandler.class.getName();
	...
	/**
	 * Configure the Tika extraction system for the pipeline in the first
	 * position after the reader (index=0).
	 * 
	...
	/**
	 * Configure the Tika extraction system for the pipeline using the command
	 * line options.
	 * 
	 * Note that the pipeline must cover at least two analysis engines (the
	 * Tika extractor and a CAS consumer) and thus the idx position may not be
	 * the last AE in the pipeline (which should be set to a CAS consumer).
	 * 
	 * @see Pipeline#configureTika(int, boolean, String, boolean, XmlHandler)
	...
	public AnalysisEngineDescription configureTika(int idx, boolean simple, CommandLine cmd)
	        throws IOException, UIMAException {
		XmlHandler handler = getTikaXmlHandler(cmd);
		String encoding = cmd.getOptionValue('e');
		boolean normalizeGreek = cmd.hasOption('g');
		return configureTika(0, simple, encoding, normalizeGreek, handler);
	}
	...
	/**
	 * Configure a simple Tika extraction system for the pipeline in the first
	 * position after the reader (index=0).
	 * 
	 * @see Pipeline#configureTika(int, boolean, String, boolean, XmlHandler)
	 */
	...
	/**
	 * Configure a simple Tika extraction system for the pipeline in the first
	 * position after the reader (index=0) using all defaults.
	 * 
	 * @see Pipeline#configureTika(int, boolean, String, boolean, XmlHandler)
	 */
	...
	/**
	 * Set the first Analysis Engine, returning the originally set AE.
	 * 
	 * <b>Note:</b> Usually, this position is occupied by the Tika AE.
	 */
	public AnalysisEngineDescription setFirst(AnalysisEngineDescription engine) {
		AnalysisEngineDescription originalEngine = pipeline[0];
		pipeline[0] = engine;
	...
	public AnalysisEngineDescription set(int index, AnalysisEngineDescription engine) {
		AnalysisEngineDescription originalEngine = pipeline[index];
	...
	/**
	 * Set all Analysis Engines, including the CAS Consumer, returning the
	 * original list of AEs.
	 */
	public AnalysisEngineDescription[] set(AnalysisEngineDescription[] engines) {
		AnalysisEngineDescription[] before = pipeline;

		if (engines.length < 1)
			throw new IllegalArgumentException("insufficient engines: " + engines.length);
	...
	/**
	 * Return the total number of (potentially not yet configured) analysis
	 * engines (without the CAS consumer)
	 */
	public int size() {
	...
	/**
	 * <code>true</code> if all AEs and the reader are set (not
	 * <code>null</code>).
	 */
	public boolean isReady() {
		...
		for (AnalysisEngineDescription engine : pipeline)
			if (engine == null)
				return false;
