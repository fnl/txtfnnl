...
public class Pipeline {
    ...
    /**
     * Add the command-line options to use a {@link JdbcConnectionResourceImpl JDBC resource} for
     * any pipeline. The added options are:
     * <ul>
     ...
     * @param numEngines number of analysis engines the pipeline will contain of without counting
     *        the "last" engine that should be the CAS consumer
     */
    ...
    /**
     * Configure a pipeline for a collection reader, one analysis engine, and a CAS consumer. The
     * all but the collection reader must be set at a later stage.
     * 
     * @param reader a collection reader configuration
     */
     ...
     * 
     * @param idx the position in the pipeline the AE should have (where 0 is the first AE after
     *        the reader)
    ...
    public AnalysisEngineDescription[] set(AnalysisEngineDescription[] engines) {
        ...
        if (engines.length < 1)
            throw new IllegalArgumentException("empty engine description array");
        pipeline = engines;
        return before;
    }

    /**
     * Return the total number of (potentially not yet configured) analysis engines (without the
     * CAS consumer) this pipeline can hold.
     */
    public int size() {
